...
def main():
    ...
    # calculate external + Hartree potential
    hartree = build_hartree_potential(rho, sz, npts)
    vext = build_external_potential(npts, sz)
    
    # calculate electron-electron repulsion energy in real-space
    E = np.einsum('ijk,ijk', hartree + vext, rho) * (sz / npts)** 3
    ana = (2 * (1 - np.sqrt(2)) / np.sqrt(np.pi))
    print('Eee = %.12f (%.12f)' % (E.real, ana))
